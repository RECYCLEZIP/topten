...
const note = {
  ...
  1: ["유색 페트병과 분리하여 배출"],
  2: ["투명 페트병과 분리하여 배출"],
  3: ["습기제거제 내부의 액체는 수분을 흡수하는 염화 칼슘이 용해되어 있으므로 배출시 수돗물과 함께 흘려 보내기"],
  ...
  8: ["펌프가 달린 뚜껑은 해제가 가능하다면 스프링과 헤드, 몸통, 튜브을 분류하여 배출", "해제가 불가능하다면 일반쓰레기에 배출"],
  ...
  14: ["나무 장난감은 일반쓰레기로 배출", "대형 완구류는 주민센터, 구청 홈페이지에서 대형 폐기물 신고 후 배출"],
  15: ["투명 페트병은 뚜껑이랑 같이 배출 가능"],
  16: ["내용물을 제거하지 않은 경우 재활용 불가능"],
};

const throwAway = {
  0: ["내용물을 비운 뒤 세척(기름기는 세제를 이용하여 제거)", "본체와 다른 재질, 상표 등을 제거", "플라스틱으로 분리 후 배출"],
  1: ["내용물을 비운 뒤 세척", "부착 상표 등을 제거", "찌그러뜨리고 뚜껑 닫기", "플라스틱으로 분리 후 배출"],
  ...
  8: ["용기 내부에 남아 있는 용액 세척", "재질이 다른 뚜껑, 상표, 설명서 등 제거", "플라스틱으로 분리 후 배출"],
  9: ["음식물 등을 제거 후 헹굼", "플라스틱의 재질의 조리용품은 플라스틱으로 분리수거"],
  10: ["이물질 깨끗하게 제거", "청소 솔과 플라스틱 손잡이 분리", "다른 재질이 없는 경우 플라스틱으로 분리 후 배출"],
  11: ["플라스틱으로 분리 후 배출"],
  12: ["내용물을 비운 뒤 씻기", "고무 패킹은 제거 후 일반쓰레기로 배출", "재질이 다른 뚜껑 둥을 제거"],
  13: ["용기 내부에 남아 있는 화장품 세척", "재질이 다른 뚜껑, 상표, 설명서 등 제거", "플라스틱으로 분리 후 배출"],
  14: ["플라스틱 외 건전지, 다른 재질 분리", "재질 별로 분리한 후 분해가 안되는 경우 일반쓰레기로 배출"],
  15: ["페트병과 페트병 뚜껑을 분리", "플라스틱 pp로 분리하여 배출"],
  16: ["비닐 뚜껑은 비닐류로 분리", "요거트 용기는 세척 후 분리", "각 재질에 맞게 분리하여 배출"],
};

const plastic = title.map((title, index) => {
  return {
    title,
