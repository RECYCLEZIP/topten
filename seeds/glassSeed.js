const title = ["유리병", "술, 탄산음료 병", "내열 유리", "도자기, 사기 그릇", "대형 유리", "형광등", "전구", "LED", "향수", "기름병"];
...
const note = {
  ...
  5: ["형광등 내부의 수은이 인체에 매우 해로운 물질", "수은이 공기 중에 유출되지 않도록 깨지지 않게 버리기"],
  6: ["깨진 전구는 신문지로 잘 감싼 뒤 불연성 쓰레기 봉투에 넣어 배출"],
  7: ["led 조명은 대부분 플라스틱이지만 유리가 포함되어 있다면 깨질 위험이 있으니 신문지에 감싸 배출"],
  8: ["용기 내부의 향수는 대부분 화학제품", "키친타올, 신문지 등으로 흡수시킨 뒤 일반쓰레기로 배출"],
...
const throwAway = {
  0: ["내용물을 비운 뒤 세척", "재질이 다른 병뚜껑, 상표 등을 제거", "병의 색깔을 구별하여 유리병 배출함에 버리기"],
  1: ["내용물을 비운 뒤 세척", "재질이 다른 병뚜껑, 상표 등을 제거", "유리병 배출함에 버리기"],
  2: ["일반 쓰레기로 배출"],
  3: ["특수 규격 봉투에 담아 배출"],
  4: ["지자체 홈페이지에서 대형폐기물 접수", "폐기물 스티커를 인쇄하거나 발급받아 부착", "집 밖으로 옮겨두면 폐기물 수거 업체에서 수거"],
  5: [
...
const glass = title.map((title, index) => {
  return {
    title,
