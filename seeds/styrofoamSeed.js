...
const note = {
  0: ["무늬, 색상이 있거나 비닐로 코팅된 스티로폼은 재활용 불가", "과일 등의 개별 포장에 쓰이는 스티로폼은 일반쓰레기로 배출"],
  1: ["라면 국물이 남아있거나, 음식물이 완전히 제거 되지 않은 컵라면 용기는 재활용 불가능하므로 일반쓰레기로 배출"],
};
const image = {
...
const throwAway = {
  0: ["내용물, 이물질을 비우고 세척", "스티로폼에 붙여져 있는 송장, 스티커 등 제거", "스티로폼으로 분리 후 배출"],
  1: ["컵라면 용기를 물로 세척", "햇빛에 하루 이상 말려 기름, 국물 자국 제거", "흰 색의 용기로 만든 뒤 배출"],
};

const styrofoam = title.map((title, index) => {
  return {
    title,
